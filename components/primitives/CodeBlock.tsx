/* Beautiful UI — Code Block. https://www.beautifului.dev
 * MIT License, Copyright (c) 2026 Shane Levine.
 *
 * Adapted for Adorable: spans the chat column instead of a fixed max width. */
"use client";
        ...
        {isDiff ? (
          <div className="relative">
            ...
            {diff.map((r, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`relative grid grid-cols-[20px_minmax(0,1fr)] items-start ${add ? "bg-green-tint" : del ? "bg-red-tint" : ""}`}
                >
                  {(add || del) && (
                    <span
                  ...
                  <span
                    className={`text-center text-[11px] select-none ${add ? "text-green" : del ? "text-red" : "text-ink-3"}`}
                  >
                    {num ?? ""}
                  </span>
          ...
          <div className="relative">
            ...
            {lines.map((line, i) => (
              ...
              >
                <span className="text-center text-[11px] text-ink-3 select-none">
                  {i + 1}
                </span>
                <code className="pr-3 pl-1 break-words whitespace-pre-wrap">
                  {highlight(line)}
